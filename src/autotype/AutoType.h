...
public slots:
    void performGlobalAutoType(const QList<QSharedPointer<Database>>& dbList, const QString& domain = QString());
    void raiseWindow();

signals:
    void globalAutoTypeTriggered(const QString& url);
    void autotypePerformed();
    void autotypeRejected();

private slots:
    void startGlobalAutoType(const QString& url);
    void unloadPlugin();
